...
import { readableStreamToString, AgrimetricsError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new AgrimetricsError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new AgrimetricsError(e, e.response?.data)
                }
                if (innerError instanceof AgrimetricsError) {
                    // Got "AgrimetricsError" from the above try block
                    throw innerError;
                }
